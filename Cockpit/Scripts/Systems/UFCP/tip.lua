...
-- TODO Set UFCP_TIP_DIR to 0 when distance to FYT is less than 2NM and aircraft is outbound, or after a FYT is changed.
...
function update_tip()
    ...
    ufcp_tip_enabled = not (UFCP_TIP_DIR:get() == 0)
    ...
    if ufcp_tip_dir == -1 then
        text = text .. "LEFT "
    elseif ufcp_tip_dir == 1 then
        text = text .. "RIGHT"
    end
    if sel == SEL_IDS.DIR then text = text .. "*" else text = text .. " " end
    ...
    if sel == SEL_IDS.ALONG and ufcp_edit_pos > 0 then text = text .. ufcp_print_edit(true) else text = text .. string.format("%4.1f", ufcp_tip_along) end
    if sel == SEL_IDS.ALONG then text = text .. "*" else text = text .. " " end
    text = text .. "NM    "
    text = text .. "\n"
    ...
    if sel == SEL_IDS.ACROSS and ufcp_edit_pos > 0 then text = text .. ufcp_print_edit(true) else text = text .. string.format("%4.1f", ufcp_tip_across) end
    if sel == SEL_IDS.ACROSS then text = text .. "*" else text = text .. " " end
    text = text .. "NM    "

    if ufcp_tip_enabled then
...
function SetCommandTip(command,value)
    ...
    elseif sel == SEL_IDS.DIR then
        if command == device_commands.UFCP_JOY_RIGHT and value == 1 then
            ufcp_tip_dir = -ufcp_tip_dir
            cursor_timer = -1
        elseif command == device_commands.UFCP_0 and value == 1 then
            ufcp_tip_enabled = not ufcp_tip_enabled
        end
        if ufcp_tip_enabled then
            UFCP_TIP_DIR:set(ufcp_tip_dir)
        else
            UFCP_TIP_DIR:set(0)
        end
