...
function cmfd_nav_load_dtc()
    ...
    if CMFD_NAV_DTC_WAYPOINT_READ:get() ~= "" then
        WAYPOINT = nil
        local ok = pcall(dofile, CMFD_NAV_DTC_WAYPOINT_READ:get())

        if ok and type(WAYPOINT) == "table" then
    ...
    if CMFD_NAV_DTC_CNTLINE_READ:get() ~= "" then
        CNT_LINE = nil
        local ok = pcall(dofile, CMFD_NAV_DTC_CNTLINE_READ:get())

        if ok and type(CNT_LINE) == "table" then
    ...
    if CMFD_NAV_DTC_FLTAREA_READ:get() ~= "" then
        FLT_AREA = nil
        local ok = pcall(dofile, CMFD_NAV_DTC_FLTAREA_READ:get())

        if ok and type(FLT_AREA) == "table" then
    ...
    if CMFD_NAV_DTC_AVDAREA_READ:get() ~= "" then
        AVD_AREA = nil
        local ok = pcall(dofile, CMFD_NAV_DTC_AVDAREA_READ:get())

        if ok and type(AVD_AREA) == "table" then
