...
ufcp_com2_power = POWER_IDS.HIGH
ufcp_com2_modulation = MODULATION_IDS.AM
ufcp_com2_sql = true
...
function update_com2()
    ...
    if ufcp_sel_format == UFCP_FORMAT_IDS.COM2 then
        ...
        if ufcp_com2_mode == MODE_IDS.OFF then
            text = text .. "OFF "
        elseif ufcp_com2_mode == MODE_IDS.TR then
            text = text .. "TR  "
        elseif ufcp_com2_mode == MODE_IDS.TR_G then
            text = text .. "TR+G"
        end
        if sel == SEL_IDS.MODE then text = text .. "*" else text = text .. " " end
        text = text .. "\n"
        ...
        if ufcp_com2_power == POWER_IDS.HIGH then
            text = text .. "HIGH"
        elseif ufcp_com2_power == POWER_IDS.MED then
            text = text .. "MED "
        elseif ufcp_com2_power == POWER_IDS.LOW then
            text = text .. "LOW "
        end
        if sel == SEL_IDS.POWER then text = text .. "*" else text = text .. " " end
        text = text .. " "
        if sel == SEL_IDS.MODULATION then text = text .. "*" else text = text .. " " end
        if ufcp_com2_modulation == MODULATION_IDS.FM then
            text = text .. "FM"
        else
            text = text .. "AM"
        end
